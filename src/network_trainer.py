...
class NetworkTrainer:
    ...
    def get_loss(self, model_output, y_target):
        model_output_transposed = model_output.transpose(1, 2)
        return self.loss_function(model_output_transposed, y_target)

    def get_accuracy(self, model_output, y_target):
        output_class_pred = model_output.argmax(dim=2)
        equal_samples = torch.eq(output_class_pred, y_target)

        return (equal_samples.sum() / constants.BATCH_SIZE) / constants.SEQUENCE_LENGTH
    ...
    def epoch_train(self, epoch):
        ...
        for batch_idx, (x, y) in enumerate(self.train_data_loader):
            ...
            (h, c) = self.network.get_initial_hidden_context() # type: ignore
            ...
            output, (h, c) = self.network(x, y, (h, c))
